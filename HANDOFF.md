...
### Wave 8 result — manager score (Fable), 2026-09-26 ~01:00, 52 shots at 15:30 in  (a-/b-/c-)
...
- **Regression to fix first (wave 9 #1): the door view for baths/tiny rooms.** Standing 0.5 m inside the door looking down 20° fills the frame with the near wall + floor and cuts the fittings (, , , ). Tried 8° pitch (): worse — fittings leave the frame entirely. So it is not the pitch: the stand point must be the far corner / diagonal that sees the most fittings from the greatest distance (the wave-7  frame), with the door view only as fallback when no corner is ≥ 1.5 m from the vanity. Help rooms: the cot is at the frame's bottom edge; look at the cot's centre from the door at eye level, or accept a 1.5 m room can't be framed and show it in the dollhouse only.
- Other wave-9 items seen in the shots: wall AC looms in the top corner of  and  (the AC belongs to the dining room, the entry stand point is in the passage — the 2.2 m rule only checks the stand point's own room); Type C entry has the wooden cube shelf 0.5 m to the right of the stand point;  faces the wall corner with the lounge chair in the foreground;  stands 0.5 m from the cabinets (Type B's kitchen frame is the model); oak wardrobes fill a third of , ,  (stand point should keep 1 m from a tall piece in frame); living-room ceiling fan fills the top of  /  (minor); sliders with invisible glass read as open (a faint sky reflection would fix it).
- Typec's rule-break list (planters get a lounge chair, L-shaped Bath-3 no shower, curtains on the study's interior glass partition, two-zone rule skips the irregular dining) still open.
