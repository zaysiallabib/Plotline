...
- **Wave-8 agent worktrees are still on disk** (`.claude/worktrees/agent-{a438…, ac79…, af5c…}` = render/typec/views, all clean and fully merged; `agent-af65…` is a stray dir holding only node_modules). Claude's auto-mode blocks `git worktree remove`; founder runs, in `E:\dev\Plotline`: `git worktree remove --force .claude/worktrees/agent-<id>` ×3, `git branch -D worktree-agent-<id>` ×3, delete `agent-af65fe6ff1691487d`, `git worktree prune`.
