...
- Waiting on the founder: (a) his bug list from walking the live link (goes first), (b) go-ahead for wave 9 #1–#3 as listed under "Next, in order".
...
### Next, in order (rewritten after wave 8, 2026-09-26)
1. **Founder deploys** (`npx vercel --prod`) and walks `/`, `/u/type-b`, `/u/type-c` and `/studio` in a real browser; his bug list goes first.
2. **Wave 9 #1 — small-room views** (see the Wave 8 result above): corner/diagonal stand point for baths and tiny rooms, door view only as fallback; entry/jump stand points keep 2.2 m from ANY wall AC in frame and 1 m from any tall piece in frame, not only the stand point's own room; kitchen jump modelled on Type B's frame; living-veranda jump faces the street or the room, never a wall corner.
3. **Wave 9 #2 — typec's rule breaks:** planters (kind balcony, name contains planter) get plants only; an L-shaped bath still gets a shower in its widest leg; curtains never on an interior glass partition (both faces indoors); the two-zone rule fires on an irregular 30 m² dining.
4. **Wave 9 #3 — polish seen in the shots:** faint sky reflection on slider glass so it reads as glass; clamp the baked sun in sky.hdr (arcs in the Bed-2 west window); ceiling fan clearance like the pendant rule.
