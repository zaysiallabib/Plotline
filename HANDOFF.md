...
5. **Third plan from ANOTHER developer** is still the real system test (Type C turned out to be Type A's own layout on floors 3/5/7). Founder sends 2–3 brochure plans from other Dhaka developers.
6. Studio follow-ups from the first round trip: small-room labels overlap at fit zoom; timer reads 00:00 after an import; Type A's own drift vs the drawing (Bath-3 is L-shaped, closet wall 0.14 m off, PDR depth) — retrace Type A in the Studio as the founder's first tracing session (masterplan week 2) and diff it against the JSON.
7. Loose ends: hood duct stops at 3.0 m (`ponytail:` note); neighbours never shade the unit; `steel_frame_shelves_01` and `drawer_cabinet` are unused (2.7 MB, delete from kit + MANIFEST + public); notes saved against old art ids no longer attach; the 5 m² open-to-sky threshold for verandas is fragile.
8. Then masterplan week 2: founder traces a unit in the Studio (record minutes), discovery demos.
